...
# 1. VISTA PRINCIPAL DEL LABORATORIO
@login_required
def laboratorio_view(request):
    # Solo mostramos la estructura, los datos se cargan por AJAX igual que en Warehouse
    return render(request, 'reparacion_warehouse/laboratorio.html')

# 2. API: INICIAR REPARACIÓN (Pendiente -> Proceso)
@login_required
@require_POST
def iniciar_reparacion(request):
    try:
        ...
        if solicitud.estado != 'pendiente':
             return JsonResponse({'status': 'error', 'message': 'Solo se pueden iniciar solicitudes pendientes'}, status=400)
             
        solicitud.estado = 'proceso'
        solicitud.save()
        
        return JsonResponse({'status': 'success', 'message': 'Reparación iniciada'})
    except Exception as e:
        return JsonResponse({'status': 'error', 'message': str(e)}, status=500)

# 3. API: FINALIZAR REPARACIÓN (Proceso -> Reparado)
@login_required
@require_POST
def finalizar_reparacion(request):
    try:
        ...
        if not solucion:
            return JsonResponse({'status': 'error', 'message': 'Debes detallar la solución técnica'}, status=400)

        if solicitud.estado != 'proceso':
             return JsonResponse({'status': 'error', 'message': 'Solo se pueden finalizar solicitudes en proceso'}, status=400)

        solicitud.estado = 'reparado' # <--- ESTADO CLAVE: Aquí Warehouse ya podrá verlo
        solicitud.solucion_tecnica = solucion
        solicitud.save()
        
        return JsonResponse({'status': 'success', 'message': 'Reparación finalizada. Equipo listo para entrega.'})
    except Exception as e:
        return JsonResponse({'status': 'error', 'message': str(e)}, status=500)